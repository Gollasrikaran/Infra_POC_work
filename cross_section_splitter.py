...
Detects graph boundaries from X-axis labels and station labels in the margin.
"""
...
class CrossSectionSplitter:
    ...
    def split(self, page, page_number, drawing_number=""):
        ...
        # grab station labels from right margin (format: XX+XX)
        ...
        # find X-axis label rows (clusters of offset numbers at same Y)
        ...
        # pair each axis row with its station label
        regions = []
        for i, axis_y in enumerate(axis_rows):
            ...
            # find closest station label within this region
            best_sta = None
            best_dist = float("inf")
    ...
    def _find_axis_rows(self, spans, page_width):
        """Find Y values where offset labels cluster (bottom of each graph)."""
        offset_spans = []
        for y, x, txt, sz in spans:
        ...
        # need at least 10 labels to count as a proper axis row
