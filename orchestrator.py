"""
Main pipeline -- ties all the cross-section processing steps together.
Classify pages, split into regions, extract profiles, compute earthwork.
"""
...
class StationResult:
    """Everything we produce for one cross-section station."""
...
class VectorPipeline:
    ...
    def run(self, pdf_path, page_numbers=None, progress_callback=None):
        ...
        # compute volumes across all stations
        if progress_callback:
    ...
    def _process_region(self, page, region):
        """Run the full pipeline on one cross-section region."""
        ...
        try:
            ...
            # transform PDF coords to engineering coords
