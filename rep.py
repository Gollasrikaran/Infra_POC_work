...
def _find_graph_regions(page: fitz.Page) -> list[fitz.Rect]:
    ...
    # Remove header/footer candidates
    peaks = np.array([
        p for p in peaks
        if 200 < p < (pix.height - 200)
    ])
    ...
    # Most sheets contain ~4 cross sections
    # ------------------------------------------------------------------
    if len(peaks) > 4:

        strengths = density[peaks]

        strongest_idx = np.argsort(strengths)[-4:]
    ...
    # ------------------------------------------------------------------
    # Build boundaries BETWEEN peaks
    # ------------------------------------------------------------------
    boundaries = []

    boundaries.append(150)
    ...
    boundaries.append(pix.height - 150)
    ...
    for i in range(len(boundaries) - 1):
        ...
        top_px = max(
            0,
            boundaries[i] - OVERLAP
        )

        bottom_px = min(
            pix.height,
            boundaries[i + 1] + OVERLAP
        )

        top_pdf = top_px * scale
        bottom_pdf = bottom_px * scale
